...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_len=500):
        ...
        div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-math.log(10000.0) / d_model)) # 공식 내: 1/(10000^(2k/d_model))
        # 짝수 채널: sin, 홀수 채널: cos
    ...
    def forward(self, x):
        ...
        """
        # 시퀀스 길이만큼 잘라서 더해줌 # transformer 입력 전 전처리 과정
...
class TimeSeriesEmbedding(nn.Module):
    ...
    def __init__(self, input_dim, d_model, max_len, dropout=0.1, use_scale=True):
        ...
        Parameters:
            - input_dim: 입력 차원
            - d_model: 임베딩 차원 # 입력값(x) 임베딩 차원 (전처리 과정에서 word embedding과 동일 역할)
            - max_len: 최대 시퀀스 길이
            - dropout: 드롭아웃 비율
        ...
        self.pos_encoder = PositionalEncoding(d_model, max_len) # 위치 인코딩 모듈 (PositionalEncoding)
...
class PVPlantTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.output_size = output_size # 최종 예측 길이 (출력 변수가 1개: 1)
